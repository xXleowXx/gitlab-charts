...
namespace :gitlab do
  namespace :docs do
    ...
    task :clean_redirects do
      ...
      #
      #   From : https://docs.gitlab.com/charts/install/requirements.html
      #   To   : /charts/install/requirements.html
      #
      def new_path(redirect, filename)
        if !redirect.start_with?('http')
          Pathname.new(filename).dirname.join(redirect).to_s.gsub(%r(\.md), '.html').gsub(%r(doc/), '/charts/')
      ...
      files_to_be_deleted.each do |filename|
        ...
        old_path = filename.gsub(%r(\.md), '.html').gsub(%r(doc/), '/charts/')
