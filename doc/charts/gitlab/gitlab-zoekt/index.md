...
gitlab-zoekt.replicas=2         # Number of Zoekt pods. If want to use one, this can be skipped
gitlab-zoekt.indexStorage=128Gi # Zoekt node disk size. Please note that Zoekt uses about x3 of the repository storage
```
...
1. Set up indexing:
   ...
   node.indices.create!(zoekt_enabled_namespace_id: enabled_namespace.id, namespace_id: namespace.id)
   ```
