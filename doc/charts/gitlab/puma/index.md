# Using the GitLab Puma Chart

The `puma` sub-chart provides the GitLab Rails webserver with three Puma workers
...
The `puma` chart is configured as follows: [Global Settings](#global-settings),
...
| `enabled`                        | `false`                | Puma enabled flag                           |
| `extraContainers`                |                       | List of extra containers to include            |
| `extraInitContainers`            |                       | List of extra init containers to include       |
| `extras.google_analytics_id`     | `nil`                 | Google Analytics Id for frontend               |
| `extraVolumeMounts`              |                       | List of extra volumes mountes to do            |
| `extraVolumes`                   |                       | List of extra volumes to create                |
| `gitlab.puma.workhorse.image` | `registry.gitlab.com/gitlab-org/build/cng/gitlab-workhorse-ee` | Workhorse image repository |
| `gitlab.puma.workhorse.tag`   |                       | Workhorse image tag                            |
| `hpa.targetAverageValue`         | `1`                   | Set the autoscaling target value               |
| `image.pullPolicy`               | `Always`              | Puma image pull policy                      |
| `image.pullSecrets`              |                       | Secrets for the image repository               |
| `image.repository`               | `registry.gitlab.com/gitlab-org/build/cng/gitlab-puma-ee` | Puma image repository |
| `image.tag`                      |                       | Puma image tag                              |
| `init.image.repository`          |                       | initContainer image                            |
| `init.image.tag`                 |                       | initContainer image tag                        |
| `memory.min`                     | `400`                 | The minimum memory threshold (in megabytes) for the Puma worker killer |
| `memory.max`                     | `650`                 | The maximum memory threshold (in megabytes) for the Puma worker killer |
...
| `replicaCount`                   | `1`                   | Puma number of replicas                     |
| `resources.requests.cpu`         | `200m`                | Puma minimum cpu                            |
| `resources.requests.memory`      | `1.4G`                | Puma minimum memory                         |
| `service.externalPort`           | `8080`                | Puma exposed port                           |
| `service.internalPort`           | `8080`                | Puma internal port                          |
| `service.name`                   | `puma`             | Puma service name                           |
| `service.type`                   | `ClusterIP`           | Puma service type                           |
...
| `shutdown.blackoutSeconds`       | `10`                  | Number of seconds to keep Puma running after receiving shutdown |
| `threads.min` | `4` | Minimum amount of puma threads per worker |
| `threads.max` | `4` | Minimum amount of puma threads per worker |
...
| `workerProcesses`                | `3`                   | Puma number of workers                      |
...
`annotations` allows you to add annotations to the Puma pods. For example:
...
`registry.gitlab.com/gitlab-org/build/cng/gitlab-puma-ce` and `workhorse.image`
...
| `ingress.tls.enabled`                  | Boolean | `true`  | When set to `false`, you disable TLS for GitLab Puma. This is mainly useful for cases in which you cannot use TLS termination at Ingress-level, like when you have a TLS-terminating proxy before the Ingress Controller. |
...
- `gitlab.puma.ingress.annotations."nginx\.ingress\.kubernetes\.io/proxy-body-size"`
- `global.ingress.annotations."nginx\.ingress\.kubernetes\.io/proxy-body-size"`

## Memory

Memory thresholds for the [puma-worker-killer](https://docs.gitlab.com/ee/administration/operations/puma.html#puma-worker-killer)
...
| `api.protocol`       | String  |                 | The protocol Puma should use to reach the Registry API. |
...
GitLab Shell uses an Auth Token in its communication with Puma. Share the token
with GitLab Shell and Puma using a shared Secret.
