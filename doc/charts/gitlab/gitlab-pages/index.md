...
| `service.sessionAffinity.enabled`         | `false`                                                    | If session affinity should be configured on the `gitlab-pages` Service (this only makes sense for traffic originating from within the cluster)                             |
| `service.sessionAffinity.config`          |                                                            | Contains the configurations of session affinity (it is the same as `Service.spec.sessionAffinityConfig`).                                                                  |
