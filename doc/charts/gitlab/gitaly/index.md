...
| `gomemlimit.enabled`                            | `true`                                             |This will automatically set the `GOMEMLIMIT` environment variable for the Gitaly container to `resources.limits.memory` iff that limit is also set. User can override this value by setting `GOMEMLIMIT` in `extraEnv`. |
