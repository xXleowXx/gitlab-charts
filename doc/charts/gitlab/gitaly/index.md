...
| `gitconfig.core.fsyncObjectFiles` | `true`                                   | Enable ordered data write properly (useful for UNIX system)                                                                                                          |
| `gitconfig.gc.auto`             | `0`                                        | Disable garbage collection for Git repository                                                                                                                        |
| `gitconfig.repack.writeBitmaps` | `true`                                     | Use to speed up the counting objects for clones and fetches                                                                                                          |
| `gitconfig.pack.threads`        | `1`                                        | Specify the number of threads to reduce packing time on multiprocesor machines                                                                                       |
| `gitconfig.pack.useSparse`      | `true`                                     | Enable sparse algorithm to obtain significant performance for computing a pack to send a small change                                                                |
| `gitconfig.receive.fsckObjects` | `true`                                     | Git-receive-pack will check all received objects                                                                                                                     |
| `gitconfig.receive.advertisePushOptions` | `true`                            | Git-receive-pack will advertise the push options capability to its clients                                                                                           |
| `gitconfig.fetch.writeCommitGraph` | `true`                                  | Enable to write commit graph after fetching                                                                                                                          |
