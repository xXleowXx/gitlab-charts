...
This class is responsible for routing a job from a Sidekiq worker to a certain queue defined in the input routing rules. The input routing rules, as described above, is an order-matter array of tuples [query, queue_name].

- The query syntax is the same as the "queue selector" detailedly denoted in doc/administration/operations/extra_sidekiq_processes.md.
...
Rules are evaluated from first to last, and as soon as we find a match for a given worker we stop processing for that worker (first match wins). If the worker doesn't match any rule, it falls back the queue name generated from the worker name. By default, the routing rules are disabled (empty array)
