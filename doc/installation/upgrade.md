...
NOTE: **Note:** If you are not using the bundled PostgreSQL chart (`postgresql.install` is false):
    - If you have supplied `global.psql.password.key`, you do not need to perform this step.
    - If you have supplied `global.psql.password.secret`, add `global.psql.password.key` to the name of your existing key to bypass this step.
