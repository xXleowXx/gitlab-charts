...
if [ -n "$env" ]; then
  ...
  if $(kubectl --namespace=$namespace get secret $rails_secret > /dev/null 2>&1); then
    ...
    enc_settings_key_base=$(fetch_rails_value secrets.yml "${env}.enc_settings_key_base")
  fi;
  ...
  enc_settings_key_base="${enc_settings_key_base:-$(gen_random 'a-f0-9' 128)}" # equavilent to secureRandom.hex(64)
...
stringData:
  secrets.yml: |-
    $env:
      ...
      enc_settings_key_base: $enc_settings_key_base
