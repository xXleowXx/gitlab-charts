...
describe 'Strategy configuration' do
  ...
  context 'When using default settings' do
    ...
    it 'Templates successfully' do
      expect(template.exit_code).to eq(0), "Unexpected error code #{t.exit_code} -- #{t.stderr}"
    end
  ...
  context 'When populating a chart strategy property' do
    ...
    it 'Templates successfully' do
      expect(local_template.exit_code).to eq(0), "Unexpected error code #{t.exit_code} -- #{t.stderr}"
    end
